...
class Window(object):
  def __init__(self, window):
    self.window=window
    self.window.wm_title("BookStore Directory")
  ...
  def get_selected_row(self, event):
    try:
      ...
      self.selected_turple=self.list1.get(index)
      self.e1.delete(0,END)
      self.e1.insert(END,self.selected_turple[1])
      self.e2.delete(0,END)
      self.e2.insert(END,self.selected_turple[2])
      self.e3.delete(0,END)
      self.e3.insert(END,self.selected_turple[3])
      self.e4.delete(0,END)
      self.e4.insert(END,self.selected_turple[4])
    except IndexError:
      pass

  def update_command(self):
    database.update(self.selected_turple[0],self.title_value.get(), self.author_value.get(),self.year_value.get(), self.isbn_value.get())
    self.clear_entry()
    self.view_command()

  def delete_command(self):
    database.delete(self.selected_turple[0])
    self.clear_entry()
    self.view_command()
